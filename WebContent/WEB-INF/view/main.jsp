<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page session= "true" import ="java.util.*, com.shshop.command.*" %>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
    <head>
    	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"> 
        <title>SHShop</title>
        <link rel="stylesheet" href="../content/css/command.css">
    </head>
    <body>
        <div class="wrap">
            <div class="header_area">
                <!--헤더 인크루트 영역-->
            </div>
            <div id="main_body">
                <div id="banner">
                    <img src="image/ben.jpg" alt="베너1">
                    <div class="control">
                        <a href="#" id="prevBtn"><img src="../content/image/left_btn.png" alt=""></a>
                        <a href="#" id="nextBtn"><img src="../content/image/right_btn.png" alt=""></a>
                    </div>
                </div>
                <div id="main_listarea">
                    <div class="main_content">
                        <p>[HOT] 인기상품</p>
                        <div class="item_area">
                            <ul>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                            </ul>
                            <div class="control">
                                <a href="#" id="prevBtn"><img src="../content/image/left_btn.png" alt=""></a>
                                <a href="#" id="nextBtn"><img src="../content/image/right_btn.png" alt=""></a>
                            </div>
                        </div>
                    </div>
                    <div class="main_content">
                        <p>[NEW] 신규상품</p>
                        <div class="item_area">
                            <ul>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                                <li><img src="http://placehold.it/110x110" alt=""></li>
                            </ul>
                            <div class="control">
                                <a href="#" id="prevBtn"><img src="../content/image/left_btn.png" alt=""></a>
                                <a href="#" id="nextBtn"><img src="../content/image/right_btn.png" alt=""></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="main_rightbox">
                    <div id="on_off_btn">
                        <img src="image/right_btn.png" alt="">
                    </div>
                    <p>최근 본 상품 리스트</p>
                    <ul>
                        <li><img src="http://placehold.it/100x100" alt=""><span>제목</span></li>
                        <li><img src="http://placehold.it/100x100" alt=""><span>제목</span></li>
                        <li><img src="http://placehold.it/100x100" alt=""><span>제목</span></li>
                    </ul>
                </div>
            </div>
            <div class="footer_area">
                <!--푸터 인크루트 영역-->
            </div>
        </div>
    </body>
</html>